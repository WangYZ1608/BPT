...
def create_model(args, num_class):
    MAE_bpt = ['MAE_bpt_vit_b', 'MAE_bpt_vit_l', 'MAE_bpt_vit_h']
    MAE_bpt_deep = ['MAE_bpt_deep_vit_b', 'MAE_bpt_deep_vit_l', 'MAE_bpt_deep_vit_h']
    MAE_bpt_svd = ['MAE_bpt_svd_vit_b', 'MAE_bpt_svd_vit_l', 'MAE_bpt_svd_vit_h']
    assert args.model_name in MAE_bpt + MAE_bpt_deep + MAE_bpt_svd
    ...
    if args.tuning_type in ["prompt"]:
        ...
        # print("msg.missing_keys: ", sorted(msg.missing_keys))
        
        requires_grad = []
        for name, param in model.named_parameters():
            if args.prompt_deep:
                if name.split('.')[0] not in ['head', 'random_vectors']:
                    if name not in ['conv1x1.8.weight',
                                    'conv1x1.9.weight',
                                    'conv1x1.10.weight',
                                    'conv1x1.11.weight',]:
    ...
    elif args.tuning_type in ["fine_tuning"]:
        ...
        if args.global_pool:
            assert set(msg.missing_keys) == {'fc_norm.weight', 'fc_norm.bias', 'head.weight', 'head.bias'}
...
def train(model, train_loader, optimizer, criterion, args, epoch, scaler, model_ema):
...
if __name__ == '__main__':  
    ...
    # model parameters
    parser.add_argument("--model_name", type=str, default="MAE_bpt_vit_b", help="architecture")
    ...
    parser.add_argument('--channels', type=int, default=75, help="conv channels for prompt positions")
    ...
    # tuning parameters
    parser.add_argument("--tuning_type", type=str, default="prompt", choices=["prompt", "fine_tuning"])
