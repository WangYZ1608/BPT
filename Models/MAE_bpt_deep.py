...
__all__ = ['MAE_bpt_deep_vit_b', 'MAE_bpt_deep_vit_l', 'MAE_bpt_deep_vit_h']


class PromptVisionTransformer(VisionTransformer):
    def __init__(self, num_prompts=100, channels=100, global_pool=True, **kwargs):
        ...
        # self.conv1x1 = nn.Sequential(*[nn.Linear(in_features=channels, out_features=self.embed_dim, bias=False) for _ in range(self.depth)])
        # init
        torch.nn.init.normal_(self.random_vectors, std=.02)
        self._init_weights(self.conv1x1)
    
    def _init_weights(self, m):
        q_path = ["./MAE_X/qk_weights/MAE_" + str(i+1) + "_q_weights.npy" for i in range(self.depth)]
        k_path = ["./MAE_X/qk_weights/MAE_" + str(i+1) + "_k_weights.npy" for i in range(self.depth)]
        x_path = ["./MAE_X/x_cubs/x" + str(i) + ".npy" for i in range(self.depth)]

        for ids, conv in enumerate(m):
            assert isinstance(conv, (nn.Conv2d, nn.Linear))
            ...
            conv.weight.data = w_.reshape(w_.size(0), w_.size(1), 1, 1).clone()   # conv layer
            # conv.weight.data = w_.clone()    # linear layer
    ...
    def _creat_prompts_linear(self, x: torch.Tensor):
        B, length, dim = x.size()

        prompt_tokens = []
        for ids in range(self.depth):
            vectors = self.random_vectors[ids].unsqueeze(0).expand(B, -1, -1)
            vectors = self.conv1x1[ids](vectors)
            prompt_tokens.append(vectors)
        return prompt_tokens
...
def MAE_bpt_deep_vit_b(**kwargs):
    model = PromptVisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def MAE_bpt_deep_vit_l(**kwargs):
    model = PromptVisionTransformer(
        patch_size=16, embed_dim=1024, depth=24, num_heads=16, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model

def MAE_bpt_deep_vit_h(**kwargs):
    model = PromptVisionTransformer(
